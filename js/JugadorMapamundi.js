function JugadorMapamundi(posicionInicialEnPixeles, estadoJuego){
    ...
    this.personaje = 5; //es fila donde esta el personaje en la imagen
...
JugadorMapamundi.prototype.aplicarEstilos = function(){
    var idHTML = "jugador";
    //document.getElementById(idHTML).style.background = "black";
...
JugadorMapamundi.prototype.comprobarColisiones = function(mapa){
    ...
    if(!this.limiteArriba.cruzar(mapa.limiteMapa)){
        this.colisionArriba = true;
    }
    if(!this.limiteAbajo.cruzar(mapa.limiteMapa)){
        this.colisionAbajo = true;
    }
    if(!this.limiteIzquierda.cruzar(mapa.limiteMapa)){
        this.colisionIzquierda = true;
    }
    if(!this.limiteDerecha.cruzar(mapa.limiteMapa)){
    ...
    for(var i = 0; i < mapa.rectangulosColisiones.length; i++){
        ...
        if(this.limiteArriba.cruzar(traduccionTemporalColision)){
            this.colisionArriba = true;
        }
        if(this.limiteAbajo.cruzar(traduccionTemporalColision)){
            this.colisionAbajo = true;
        }
        if(this.limiteIzquierda.cruzar(traduccionTemporalColision)){
            this.colisionIzquierda = true;
        }
        if(this.limiteDerecha.cruzar(traduccionTemporalColision)){
            this.colisionDerecha = true;
        }
    }
